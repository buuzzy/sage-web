import { importBackupData } from '@/shared/db';
...
export async function incrementalCloudSync(): Promise<void> {
  ...
  if (sessionsError) {
    console.warn(
      '[CloudSync] Failed to fetch new sessions:',
  ...
  console.log(
    `[CloudSync] Found ${newSessions.length} new/updated sessions, syncing...`
  );
  ...
  ] = await Promise.all([messagesPromise, filesPromise]);

  if (messagesError) {
    console.warn(
      '[CloudSync] Failed to fetch messages:',
  ...
  const normalizedSessions = (newSessions as CloudSessionRow[]).map(
    (session) => ({
      id: session.id,
  ...
  console.log(
    `[CloudSync] Incremental sync done: ${result.sessions} sessions, ${result.tasks} tasks, ${result.messages} messages, ${result.files} files`
  );
}
