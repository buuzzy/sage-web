...
⚠️ 再次强调输出格式：你的输出必须是一个包含 "profile" 和 "recent_threads" 两个顶层 key 的 JSON 对象。不要直接输出 profile 的内容（如直接以 "explicit" 开头），必须包裹在 "profile" key 下。正确示例：{"profile": {...}, "recent_threads": [...]}`;
...
async function callDistillLlm(
  ...
  if (!result.profile && (result as any).explicit) {
    finalResult = {
      profile: {
        explicit: (result as any).explicit,
        implicit: (result as any).implicit,
      },
      recent_threads: (result as any).recent_threads ?? [],
    } as DistillResult;
    console.warn(
      '[distill] LLM returned unwrapped profile structure, auto-recovered'
    );
  }
  ...
  return {
    ...
    recent_threads: finalResult.recent_threads.slice(0, RECENT_THREADS_LIMIT),
  };
}
