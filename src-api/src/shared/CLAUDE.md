...
| provider/ | manager.ts, user-store.ts, types.ts | 模型 Provider 管理（manager 单例；user-store 读取 user_providers 表） | 🔒 接口稳定 |
| skills/ | loader.ts, register.ts, predictor.ts, config.ts | 技能加载、注册、意图预测 | 🔧 |
| supabase/ | client.ts | Supabase client（双模式：service-role / user-JWT） | 🔒 |
| cron/ | scheduler.ts, store.ts, types.ts | Cron 任务调度（用户定时任务） | 🔧 |
| services/ | agent.ts, chat.ts, preview.ts, conversation.ts | 业务服务层 | 🔧 |
| init/ | first-run.ts, migration.ts, sandbox-migration.ts | 启动初始化 + 数据迁移 | 🔧 |
| mcp/ | loader.ts | MCP 配置加载（~/.sage/mcp.json + ~/.claude/settings.json） | 🔒 |
| utils/ | logger.ts, paths.ts, url.ts, config.ts | 通用工具 | 🔒 |
...
- `agent.ts`：`getAgent()` 获取/创建全局 Agent 实例 + `runAgent()` 单路径执行
